...
// Mockup — Moment 4 (What goes in the bag) with cohort toggle.
//
// Mirrors the canonical Moment4Page pattern: single-select action chips
// for bag composition. The chip catalog is universal (5 common bag
// patterns); cohort variation is in the captured profile state (M1+M2+M3
// pre-filled), prior history, and the typical pre-selected chip + draft.
...
// Universal bag composition catalog — same across cohorts.
const bagPatterns: ChipOption[] = [
  { key: 'main-1-side', label: 'Main + 1 side' },
...
const LUMI_PROSE =
  "And how does lunch travel for the kids? One main plus a side, a thermos, a bento — pick what feels closest, or describe a different pattern.";
...
const ANGLO_PRIOR: ChatTurn[] = [
  ...
  },
  {
    ...
    role: 'parent',
    content:
      'Pretty straightforward — Anglo, some Mediterranean. Sandwiches, pasta, grilled chicken. Sam loves anything with cheese.',
  },
];
...
const SOMALI_PRIOR: ChatTurn[] = [
  ...
  },
  {
    ...
    role: 'parent',
    content:
      "We&rsquo;re Somali. Halal of course. Cooking is mostly traditional — anjero, sambusas, basbaas, suugo on the bariis. Kids will eat plain rice or fruit on a slow day, but I want them eating what they grew up with.",
  },
  {
...
const COHORTS: Record<CohortId, CohortFixture> = {
  anglo: {
    ...
    desc: 'Single child; simple household-wide bag pattern.',
  ...
  somali: {
    ...
    desc: 'Two children, same bag pattern (main + 1 side); thermos common for warm dishes.',
    ...
    initialChip: 'main-1-side',
    initialDraft: 'Same for both — main + one side. They take a thermos for the warm dishes.',
...
const initialState: Record<CohortId, CohortState> = {
  anglo: { selectedChip: COHORTS.anglo.initialChip, draft: COHORTS.anglo.initialDraft },
  somali: { selectedChip: COHORTS.somali.initialChip, draft: COHORTS.somali.initialDraft },
};

// ─── Page ─────────────────────────────────────────────────────────────────

export function Moment4PersonalizedPage() {
  ...
  const selectedChip = state.selectedChip;
  ...
  const historyTurns: ChatTurn[] = useMemo(
    () => [...cohort.priorHistory, { id: `${cohort.id}-m4-lumi`, role: 'lumi', content: LUMI_PROSE }],
  ...
  function handleChipTap(key: string) {
    // Single-select toggle; tap same chip again to deselect.
    updateCohortState(activeCohort, { selectedChip: selectedChip === key ? null : key });
  }

  function handleSend() {
    updateCohortState(activeCohort, { selectedChip: null, draft: '' });
  }

  const placeholder = 'Add detail — different for each kid, or anything specific about how lunch looks…';

  const selectedLabel = selectedChip ? bagPatterns.find((p) => p.key === selectedChip)?.label ?? null : null;
          ...
          <header className="shrink-0 flex items-center justify-between bg-bg/80 px-6 md:px-8 py-5 backdrop-blur-sm">
            <div className="flex flex-col gap-1">
              ...
              <span className="font-sans text-[11px] font-medium uppercase tracking-widest text-fg-muted">
                Moment 4 of 5 · What goes in the bag · <span className="text-fg">personalized</span>
              </span>
            </div>
            ...
            <div className="flex flex-1 flex-col items-center justify-start overflow-y-auto px-6 md:px-8 py-8 min-h-0">
              <div className="flex flex-col items-center gap-6 text-center max-w-2xl w-full">
                ...
                <div className="flex w-full flex-col items-center gap-2 pt-1">
                  ...
                  <div role="radiogroup" aria-label="Bag composition" className="flex flex-wrap justify-center gap-2">
                    {bagPatterns.map((opt) => (
                      <ChoiceChip
                        key={opt.key}
                        label={opt.label}
          ...
          >
            ...
            {!hasResponse ? (
              <p className="mt-2 text-center font-sans text-xs italic text-amber/80">
                Required — pick a pattern, or describe how lunch travels for the kids.
              </p>
            ) : selectedChip ? (
              <p className="mt-2 text-center font-sans text-xs italic text-foliage">
                {selectedLabel} — will be sent with your message
              </p>
            ) : (
              <p className="mt-2 text-center font-sans text-xs italic text-foliage">
                Free-text response — Lumi will parse the pattern from what you say.
              </p>
            )}
          </form>
        </section>
...
function KitchenProfilePanel({ cohort, selectedBag }: { cohort: CohortFixture; selectedBag: string | null }) {
  const moment4Done = selectedBag !== null;
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-4 flex flex-col gap-3">
        ...
        <ProfileCard active icon={<IcoGlobe cls="h-4 w-4 shrink-0 text-amber-soft" />} title="Your kitchen's taste">
          <div>
        ...
        <ProfileCard active={moment4Done} icon={<IcoLunchBag cls="h-4 w-4 shrink-0 text-amber-soft" />} title="What goes in the bag" waitingLabel="What goes in the bag">
          {selectedBag ? (
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-2.5 md:flex-row md:items-center md:justify-between">
        <div className="flex flex-col gap-0.5">
          <span className="font-sans text-[10px] uppercase tracking-[0.18em] text-memory-provenance-500">
            Pre-flight mockup · Moment 4 (what goes in the bag) · cohort toggle
          </span>
