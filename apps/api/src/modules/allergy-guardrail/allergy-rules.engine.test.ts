...
// The 9 FALCPA seed rules. As of guardrail 1.4.0 these NO LONGER block on their
// own — they satisfy the baseline-presence sanity check and feed synonym
// expansion, but hard blocks come only from parent_declared rules.
...
// A household that has DECLARED every FALCPA-9 allergen (household-wide). Used by
// the matcher/synonym tests: includes both the falcpa seed (so the baseline
// check passes) AND a parent_declared rule per allergen (so matches hard-block
// under 1.4.0). conflict.allergen keys are identical to the old floor behaviour.
const ALL_DECLARED: AllergyRule[] = [
  ...FALCPA_BASELINE,
  ...FALCPA_BASELINE.map((r) => ({ ...r, rule_type: 'parent_declared' as const })),
];
...
describe('allergy-rules.engine.evaluate', () => {
  ...
  it('returns blocked when a declared allergen name appears as a substring in an ingredient', () => {
    const items = [item({ ingredients: ['crushed peanuts'] })];
    const result = evaluate(items, ALL_DECLARED);
  ...
  it('household-wide declared rule (child_id = null) blocks items for any child in the plan', () => {
    const rules = [
      ...FALCPA_BASELINE,
      rule({ allergen: 'peanut', child_id: null, rule_type: 'parent_declared' }),
    ];
    const items = [
  ...
  // -------------------- Guardrail 1.4.0 — Option A: no FALCPA floor on recipes -----

  it('clears an undeclared FALCPA allergen in a MAIN slot (no floor on recipes)', () => {
    // 'pasta' (wheat) and 'cheddar cheese' (dairy) are FALCPA synonyms, but this
    // household declared neither — under 1.4.0 they must clear on recipe slots.
    const items = [item({ slot: 'main', ingredients: ['pasta', 'cheddar cheese'] })];
    const result = evaluate(items, FALCPA_BASELINE);
    expect(result).toEqual({ verdict: 'cleared', conflicts: [] });
  });

  it('still blocks an undeclared-but-DECLARED allergen on a recipe slot', () => {
    // Same wheat ingredient, but now wheat IS declared household-wide → blocked.
    const rules = [...FALCPA_BASELINE, rule({ allergen: 'wheat', rule_type: 'parent_declared' })];
    const items = [item({ slot: 'main', ingredients: ['pasta'] })];
    const result = evaluate(items, rules);
    expect(result.verdict).toBe('blocked');
    if (result.verdict === 'blocked') {
      expect(result.conflicts[0].allergen).toBe('wheat');
    }
  });

  // -------------------- bug fix: matcher false positives --------------------

  it('does NOT flag "ground beef" as peanut (the "ground" ⊂ "groundnut" false positive)', () => {
    const items = [item({ ingredients: ['ground beef', ...SAFE_INGREDIENTS] })];
    const result = evaluate(items, ALL_DECLARED);
    expect(result).toEqual({ verdict: 'cleared', conflicts: [] });
  });

  it('still flags genuine peanut synonyms ("groundnut paste") when peanut is declared', () => {
    const items = [item({ ingredients: ['groundnut paste'] })];
    const result = evaluate(items, ALL_DECLARED);
    expect(result.verdict).toBe('blocked');
    if (result.verdict === 'blocked') {
      expect(result.conflicts[0].allergen).toBe('peanut');
    }
  });

  it('flags "mixed nuts" as tree_nut via the word-level "nuts" match', () => {
    const items = [item({ ingredients: ['mixed nuts'] })];
    const result = evaluate(items, ALL_DECLARED);
    expect(result.verdict).toBe('blocked');
    if (result.verdict === 'blocked') {
      expect(result.conflicts.some((c) => c.allergen === 'tree_nut')).toBe(true);
    }
  });

  // -------------------- D1: FALCPA synonym/alias matching (P15) --------------------
  // Synonym expansion is unchanged by 1.4.0 — it just now requires the allergen
  // to be DECLARED (ALL_DECLARED) rather than relying on the removed floor.

  it('declared "tree_nut" blocks "almonds" (synonym match)', () => {
    const items = [item({ ingredients: ['almonds'] })];
    const result = evaluate(items, ALL_DECLARED);
    expect(result.verdict).toBe('blocked');
    if (result.verdict === 'blocked') {
      expect(result.conflicts.some((c) => c.allergen === 'tree_nut' && c.ingredient === 'almonds')).toBe(true);
    }
  });

  it('declared "fish" blocks "grilled salmon" (synonym match)', () => {
  ...
  it('declared "dairy" blocks "cheddar cheese" (synonym match)', () => {
  ...
  it('declared "wheat" blocks "pasta" (synonym match)', () => {
    const items = [item({ ingredients: ['pasta'] })];
    const result = evaluate(items, ALL_DECLARED);
    expect(result.verdict).toBe('blocked');
  });

  it('declared "egg" blocks "mayonnaise" (synonym match)', () => {
    const items = [item({ ingredients: ['mayonnaise'] })];
    const result = evaluate(items, ALL_DECLARED);
    expect(result.verdict).toBe('blocked');
  });

  it('declared "soy" blocks "tofu" (synonym match)', () => {
    const items = [item({ ingredients: ['tofu'] })];
    const result = evaluate(items, ALL_DECLARED);
    expect(result.verdict).toBe('blocked');
  });

  it('declared "shellfish" blocks "shrimp" (synonym match)', () => {
    const items = [item({ ingredients: ['shrimp'] })];
    const result = evaluate(items, ALL_DECLARED);
    expect(result.verdict).toBe('blocked');
  });

  it('declared "sesame" blocks "tahini" (synonym match)', () => {
    const items = [item({ ingredients: ['tahini'] })];
    const result = evaluate(items, ALL_DECLARED);
    expect(result.verdict).toBe('blocked');
  });

  it('declared "peanut" blocks "groundnut paste" (synonym match)', () => {
    const items = [item({ ingredients: ['groundnut paste'] })];
    const result = evaluate(items, ALL_DECLARED);
    expect(result.verdict).toBe('blocked');
  });
  ...
  it('blocks declared peanut in snack slot even when school policy scope is main-only', () => {
  ...
  // -------- Guardrail 1.4.0 — declared-only on every slot (snack + recipe) --------
  // Parent-curated snack SKUs AND recipe slots are evaluated against
  // parent_declared rules only. A dairy- or wheat-tagged item clears when no
  // child declared that allergen, on ANY slot, while a declared allergen still
  // blocks everywhere.
  it('clears a dairy-tagged snack SKU when no child declared dairy', () => {
    const items = [
  ...
  it('clears a dairy ingredient in a MAIN slot when dairy is not declared', () => {
    const items = [
  ...
  it('declared "fish" blocks "worcestershire" (3.24 synonym, anchovy-based)', () => {
  ...
  it('declared "sesame" blocks "za\'atar" (3.24 synonym)', () => {
    const items = [item({ ingredients: ["za'atar"] })];
    const result = evaluate(items, ALL_DECLARED);
    expect(result.verdict).toBe('blocked');
    if (result.verdict === 'blocked') {
      expect(result.conflicts.some((c) => c.allergen === 'sesame')).toBe(true);
    }
  });

  it('declared "sesame" blocks "zaatar" (3.24 synonym, unaccented)', () => {
...
describe('FALCPA synonym expansion — canonical key coverage', () => {
  ...
  // from these tests so the parent_declared path is isolated. One unrelated
  // falcpa rule satisfies the engine's fail-closed baseline check.
