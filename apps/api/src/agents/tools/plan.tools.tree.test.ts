...
import { createPlanComposeTreeSpec } from './plan.tools.js';
import { PLANNER_PROMPT_TREE } from '../prompts/planner.prompt.js';
import type { PlansService } from '../../modules/plans/plans.service.js';

// Story 3-DM-C1 Phase 5 — §10.7's "synthetic plan_compose gate" surfaced as a
// Vitest test: stub the planner LLM with hand-authored tree-shape outputs,
// drive them through the plan.compose.tree tool, assert the input/output
...
describe('PLANNER_PROMPT_TREE', () => {
  it('declares v2.0.0 and includes plan.compose.tree in toolsAllowed', () => {
    expect(PLANNER_PROMPT_TREE.version).toBe('v2.0.0');
    expect(PLANNER_PROMPT_TREE.toolsAllowed).toContain('plan.compose.tree');
    expect(PLANNER_PROMPT_TREE.toolsAllowed).not.toContain('plan.compose');
  });

  it('embeds the worked examples inline in the prompt text', () => {
    expect(PLANNER_PROMPT_TREE.text).toContain('main_assignments');
    expect(PLANNER_PROMPT_TREE.text).toContain('main_assignment_sequence');
    expect(PLANNER_PROMPT_TREE.text).toContain('Example 1 — Shared Main Monday');
    expect(PLANNER_PROMPT_TREE.text).toContain('Example 2 — Allergen-fork Wednesday');
    expect(PLANNER_PROMPT_TREE.text).toContain('plan.compose.tree');
  });
});

describe('createPlanComposeTreeSpec — synthetic plan_compose round-trips', () => {
  ...
  it('accepts the shared-Main day output through input + output schema parse', async () => {
    const spec = createPlanComposeTreeSpec(buildPlansServiceStub(), buildRedisStub());
  ...
  it('routes the shared-Main day to plansService.composeTree and returns a tree-shape output', async () => {
    ...
    const spec = createPlanComposeTreeSpec(service, buildRedisStub());
