...
**Status:** ⚠️ SUPERSEDED (v1 below describes the deprecated ElevenLabs ConvAI model)
**Version:** 1.1
**Feature Area:** Voice
**Module:** `voice/`
**Author:** HiveLume Engineering
**Last Updated:** 2026-06-07
**Changelog:** 2026-06-07 — v2 direction banner added; v1 body (ConvAI Agent + webhook) deprecated.

---

> ## ⚠️ SUPERSEDED — read this first (v2 direction, 2026-06-07)
>
> **The body of this document (v1.1, §1–§13 below) describes the deprecated
> ElevenLabs Conversational AI ("ConvAI") Agent + inbound-webhook model. That
> model is no longer used.** The canonical, as-built voice pipeline is:
>
> - **ElevenLabs is two stateless audio services only — STT and TTS.** There is
>   NO ElevenLabs Agent: no `agent_id` / `get_signed_url`, no ElevenLabs-hosted
>   LLM, no ElevenLabs custom-LLM webhook, and **no inbound ElevenLabs webhook /
>   signing secret** (`POST /v1/voice/webhook/elevenlabs` and `POST /v1/voice/token`
>   / `POST /v1/voice/llm` are removed — they 404).
> - **HiveKitchen owns the realtime transport**: the browser opens HiveKitchen's
>   own WebSocket `GET /v1/voice/ws?session_id=…&token=<jwt>`. Session creation is
>   `POST /v1/voice/sessions`. Raw audio never reaches OpenAI and is never stored.
> - **Turn management is client + server, not ElevenLabs.** The browser does VAD
>   (`@ricky0123/vad-react`) and sends a complete WAV utterance over the WS.
> - **STT:** ElevenLabs Scribe (`scribe_v1`) via REST `POST /v1/speech-to-text`
>   (synchronous; the transcript is the HTTP response — NOT a webhook).
> - **TTS:** ElevenLabs `/v1/text-to-speech/{voice}/stream` (server-side, `eleven_v3`)
>   for replies, or the browser-direct `/stream-input` WS with a single-use token
>   for narration (slice 2-s20).
> - **The reply is generated by HiveKitchen's OpenAI agent** (OnboardingAgent /
>   LumiAgent), never by ElevenLabs.
>
> **Reference implementation: story 2.6b** (Voice Pipeline v2 — HK-Owned WebSocket,
> ElevenLabs STT + TTS), which replaced story 2.6's Custom-LLM model.
>
> Known debt: the ambient Lumi tap-to-talk path (12-S10 / 12.5) still mints ConvAI
> signed URLs for STT and is being migrated to raw Scribe+TTS to match 2.6b.
>
> Everything below is retained for historical context only. Do not implement from it.
