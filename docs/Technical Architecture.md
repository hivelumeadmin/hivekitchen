...
Voice exists as a persistent, optional input area — not a full-screen or conversation-first interface. The frontend calls `POST /v1/voice/sessions` to create a session, then opens **HiveKitchen's own WebSocket** (`GET /v1/voice/ws?session_id=…&token=<jwt>`); the browser does client-side VAD and streams WAV utterances over that WS, receiving synthesised audio back over the same WS. ElevenLabs is used server-side for STT (Scribe) and TTS only — there is no ElevenLabs SDK on the frontend and no ElevenLabs Agent. The voice overlay shows session state and can expand to show the live transcript on user tap. When a voice session ends, the parent can continue by text on the same thread with no loss of context.
...
HiveKitchen is a REST API with a Server-Sent Events (SSE) gateway for real-time delivery and a dedicated voice WebSocket (`GET /v1/voice/ws`). It is the sole orchestrator of the system — it handles auth, conversation threading, AI agent dispatch, resource persistence, and the voice audio bridge (ElevenLabs STT/TTS). All access to the Data Layer is owned exclusively by this layer.
...
| **Voice Service** 		| Session lifecycle, HiveKitchen voice WebSocket, ElevenLabs STT/TTS calls |
...
All traffic runs over TLS. JWT access tokens carry a short TTL with refresh token rotation on every use. The voice WebSocket authenticates via a JWT passed in the `GET /v1/voice/ws` query string (browsers cannot set headers on WS upgrades). Rate limiting is enforced at the gateway layer using Redis. Resources are strictly user-scoped — no cross-user data access is possible.

---

## ElevenLabs Integration

> **Updated 2026-06-07 — off the ElevenLabs Agent.** ElevenLabs is used as **two
> stateless audio services only — STT and TTS**. There is no ElevenLabs
> Conversational AI ("ConvAI") Agent, no `agent_id`/`get_signed_url`, no
> ElevenLabs-hosted LLM, and **no inbound ElevenLabs webhook / signing secret**.
> Reference implementation: story **2.6b**.

ElevenLabs owns only audio transcription and synthesis. **HiveKitchen owns the
realtime transport** (its own WebSocket `GET /v1/voice/ws`), session lifecycle,
and turn management. The browser does client-side VAD and sends complete
utterances; the conversational reply is always generated by HiveKitchen's OpenAI
agent. Raw audio never reaches OpenAI and is never persisted.

### Stack & Configuration

| Setting 						| Value |
| --- 							| --- |
| Transport 					| HiveKitchen-owned WebSocket (`GET /v1/voice/ws`); browser sends WAV utterances, receives audio frames |
| STT 							| ElevenLabs Scribe (`scribe_v1`) via REST `POST /v1/speech-to-text` — synchronous, NOT a webhook |
| TTS 							| ElevenLabs `/v1/text-to-speech/{voice}/stream` (`eleven_v3`, server-side); `/stream-input` WS for narration |
| Turn detection 				| Client-side VAD (`@ricky0123/vad-react`); server processes complete utterances |
| Reply generation 				| HiveKitchen OpenAI agent (OnboardingAgent / LumiAgent) — ElevenLabs never generates text |

### Voice Session Flow

`User activates voice  →  Client calls POST /v1/voice/sessions (returns session_id)
                      →  Client opens HiveKitchen WS: GET /v1/voice/ws?session_id=…&token=<jwt>
                      →  Client VAD captures an utterance, sends a WAV frame over the WS
                      →  HiveKitchen calls ElevenLabs Scribe STT (REST), gets transcript
                      →  HiveKitchen runs the OpenAI agent pipeline, persists resources
                      →  HiveKitchen calls ElevenLabs TTS (REST), forwards audio frames over the WS
                      →  HiveKitchen writes both turns to the thread`

### Session Lifecycle

A session opens when the client calls `POST /v1/voice/sessions` and opens the
HiveKitchen WebSocket. It closes either cleanly — the user ends the session and
the WS closes — or via disconnection, detected server-side by the WS `onclose`
handler (no webhook awaited); HiveKitchen marks the session `disconnected`. In
either case, persisted resources are retained and the transcript remains readable
