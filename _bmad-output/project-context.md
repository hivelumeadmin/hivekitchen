...
**Voice pipeline invariants** _(updated 2026-06-07 — off the ElevenLabs Agent)_
- **ElevenLabs is two stateless audio services only — STT and TTS.** HiveKitchen owns the realtime transport, session lifecycle, and turn management. There is **no ElevenLabs Conversational AI ("ConvAI") Agent**: no `agent_id` / `get_signed_url`, no ElevenLabs-hosted LLM, no ElevenLabs custom-LLM webhook, and no ElevenLabs post-call webhook / signing secret (`xi-signature`). The reference implementation is story **2.6b** (onboarding).
  - **STT:** ElevenLabs Scribe (`scribe_v1`) via REST `POST /v1/speech-to-text`. The browser captures audio (client-side VAD, `@ricky0123/vad-react`), sends a complete WAV utterance over HiveKitchen's own WebSocket (`GET /v1/voice/ws`), and the API calls Scribe and gets the transcript synchronously. STT delivery is request/response — **not** a webhook.
  - **TTS:** ElevenLabs `/v1/text-to-speech/{voice}/stream` (REST, server-side, `eleven_v3`) for conversational replies, or the browser-direct `/v1/text-to-speech/{voice}/stream-input` WebSocket with a single-use token for narration (slice 2-s20). HiveKitchen forwards audio to the browser as binary WS frames.
- **The conversational reply is always generated by HiveKitchen's OpenAI agent** (OnboardingAgent / LumiAgent), never by ElevenLabs.
...
- KNOWN DEBT: the ambient Lumi tap-to-talk path (12-S10 / 12.5) still mints ConvAI Agent signed URLs for STT in `lumi.service.ts` — being migrated to raw Scribe+TTS to match 2.6b. Do not copy that pattern.
