...
- Voice: ElevenLabs used exclusively for STT and TTS via **browser-direct WebSocket connections** with single-use tokens (per ADR-002 — Ambient Lumi, 2026-04-29). HiveKitchen owns the LLM layer (OpenAI), turn management, thread persistence, and token issuance. No ElevenLabs Conversational AI SDK. Two separate browser-direct WebSocket connections per tap-to-talk session: STT (`wss://api.elevenlabs.io/v1/Speech-to-Text/?single_use_token={token}`) and TTS (`wss://api.elevenlabs.io/v1/text-to-speech/{voice_id}/stream-input?single_use_token={token}`). HiveKitchen API issues one STT token + one TTS token per talk session (`POST /v1/lumi/voice/sessions`); the permanent ElevenLabs API key never reaches the browser. Raw audio never transits the HiveKitchen API — only text crosses the HiveKitchen boundary. No inbound ElevenLabs webhook.
...
> ⚠️ **SUPERSEDED 2026-06-07 — voice pipeline went off the ElevenLabs Agent.**
> Every voice reference in this document that assumes an ElevenLabs Conversational
> AI ("ConvAI") Agent, a `POST /v1/voice/token` signed-URL flow, an inbound
> `POST /v1/webhooks/elevenlabs` webhook, or `X-Elevenlabs-Signature` HMAC
> validation is **deprecated** (this §3.4, §3.5's "synchronous webhook response,"
> §3.5 voice-webhook-payload, the §1 "ElevenLabs webhook latency" rationale, the
> voice rows in the integration/data-flow tables, and `POST /v1/voice/token`).
> The canonical model: ElevenLabs is **STT (Scribe REST) + TTS only**; HiveKitchen
> owns its own voice WebSocket (`GET /v1/voice/ws`) and session lifecycle
> (`POST /v1/voice/sessions`); the reply is generated by HiveKitchen's OpenAI
> agent; there is no ElevenLabs webhook or signing secret. Reference: story 2.6b
> and `docs/Technical Architecture.md → ElevenLabs Integration`. **NOTE:** the
> `/v1/webhooks/stripe` HMAC webhook and the Lunch Link signed-URL HMAC (§2.3) are
> unrelated and remain valid — this notice applies to the ElevenLabs/voice
> references only.
